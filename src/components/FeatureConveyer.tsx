...
export default function FeatureConveyer({
  ...
  // Starts at top of right reel (481, 209), loops clockwise around right reel,
  // exits bottom-left and runs along bottom (y = 281) under the projector body,
  // enters Left Reel (319, 281) from bottom, loops clockwise around it, and terminates at top.
  const pathD =
    "M 481,209 C 517,209 517,281 481,281 C 445,281 445,209 481,209 C 517,209 517,281 481,281 C 450,281 420,281 400,281 C 380,281 350,281 319,281 C 283,281 283,209 319,209 C 355,209 355,281 319,281 C 283,281 283,209 319,209";
  ...
  useEffect(() => {
    ...
    const updatePositions = () => {
      ...
      if (reelRef.current) {
        reelRef.current.style.transform = `rotate(${progress.current * 0.5}deg)`;
      }

      if (leftReelRef.current) {
        leftReelRef.current.style.transform = `rotate(${progress.current * 0.5}deg)`;
      }
      ...
      conveyerItems.forEach((_, index) => {
        ...
        // Find which item is closest to the bottom-center projector scan line (centered at x = 400, y is not restricted to make it robust)
        const isNearGate = pt.x > 300 && pt.x < 500;
        if (isNearGate) {
    ...
    <div className="w-full overflow-visible relative flex justify-center h-[300px]">
      ...
      >
          ...
          >
            ...
            <g ref={reelRef} style={{ transformOrigin: "481px 245px" }}>
              <circle
                cx="481"
                cy="245"
                r="36"
              ...
              <circle
                ...
                strokeWidth="1"
              />
              <circle cx="481" cy="245" r="10" fill="#24272D" />
              <circle cx="481" cy="245" r="3" fill="#B29B7C" />
              {[0, 60, 120, 180, 240, 300].map((angle) => (
                <circle
                  key={angle}
                  cx={481 + 20 * Math.cos((angle * Math.PI) / 180)}
            ...
            <g ref={leftReelRef} style={{ transformOrigin: "319px 245px" }}>
              <circle
                cx="319"
                cy="245"
                r="36"
              ...
              <circle
                ...
                strokeWidth="1"
              />
              <circle cx="319" cy="245" r="10" fill="#24272D" />
              <circle cx="319" cy="245" r="3" fill="#B29B7C" />
              {[0, 60, 120, 180, 240, 300].map((angle) => (
                <circle
                  key={angle}
                  cx={319 + 20 * Math.cos((angle * Math.PI) / 180)}
                  cy={245 + 20 * Math.sin((angle * Math.PI) / 180)}
                  r="5"
                  fill="white"
                  stroke="#24272D"
            ...
            {conveyerItems.map((item, index) => {
              ...
              return (
                <div
                  ...
                  className="absolute flex flex-col items-center pointer-events-auto transition-opacity duration-150 z-20"
                  style={{
                    left: "50%",
                    top: "50%",
